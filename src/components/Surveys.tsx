...
const Surveys: React.FC = () => {
    ...
    // Prepare chart data for the first question
    const chartData = selectedSurvey?.results ? Object.entries(selectedSurvey.results).map(([name, value]) => ({ name, value })) : [];
                ...
                <div className="fixed inset-0 bg-slate-900/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
                    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl h-[85vh] overflow-hidden flex flex-col">
                        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                            <div>
                                <h3 className="font-bold text-xl text-slate-800">{selectedSurvey.title}</h3>
                                <p className="text-sm text-slate-500">{selectedSurvey.responseCount} respostas recebidas</p>
                            </div>
                            <button onClick={() => setActiveView('LIST')} className="p-2 hover:bg-slate-200 rounded-full transition-colors"><X size={24} className="text-slate-500"/></button>
                        </div>
                        <div className="flex-1 overflow-y-auto p-8 bg-slate-50">
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 mb-6">
                                <h4 className="font-bold text-slate-700 mb-6 border-b pb-2">Resultados Consolidados (Q1)</h4>
                                {chartData.length > 0 ? (
                                    <div className="h-80">
                                    ...
                                    <div className="text-center py-10 text-slate-400">Ainda não há dados suficientes para gráficos.</div>
                                )}
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="bg-white p-6 rounded-2xl border border-slate-200">
                                    <h5 className="font-bold text-slate-700 mb-4">Detalhes</h5>
                                    <ul className="space-y-3">
                                        <li className="flex justify-between text-sm border-b border-slate-50 pb-2"><span className="text-slate-500">Status</span> <span className="font-bold text-emerald-600">Ativo</span></li>
                                        <li className="flex justify-between text-sm border-b border-slate-50 pb-2"><span className="text-slate-500">Início</span> <span className="font-bold text-slate-700">{new Date(selectedSurvey.startDate).toLocaleDateString()}</span></li>
                                        <li className="flex justify-between text-sm border-b border-slate-50 pb-2"><span className="text-slate-500">Fim</span> <span className="font-bold text-slate-700">{new Date(selectedSurvey.endDate).toLocaleDateString()}</span></li>
                                        <li className="flex justify-between text-sm border-b border-slate-50 pb-2"><span className="text-slate-500">Acesso Externo</span> <span className="font-bold text-indigo-600">{selectedSurvey.externalAccess ? 'Sim' : 'Não'}</span></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
            ...
            {isCreateOpen && (
                 ...
                 <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden animate-scale-in flex flex-col max-h-[90vh]">
                     ...
                     <div className="p-6 overflow-y-auto flex-1 space-y-5">
                         ...
                         <div><label className="text-xs font-bold text-slate-500 uppercase mb-1 block">Descrição</label><textarea value={newSurvey.description || ''} onChange={e => setNewSurvey({...newSurvey, description: e.target.value})} className="w-full px-4 py-3 bg-slate-50 border rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500" rows={3}/></div>
                         <div className="grid grid-cols-2 gap-4">
                             <div><label className="text-xs font-bold text-slate-500 uppercase mb-1 block">Tipo</label><select value={newSurvey.type} onChange={e => setNewSurvey({...newSurvey, type: e.target.value as any})} className="w-full px-4 py-3 bg-slate-50 border rounded-xl text-sm"><option value="SATISFACTION">Pesquisa</option><option value="VOTING">Votação</option><option value="CENSUS">Censo</option></select></div>
                             <div><label className="text-xs font-bold text-slate-500 uppercase mb-1 block">Fim</label><input type="date" value={newSurvey.endDate} onChange={e => setNewSurvey({...newSurvey, endDate: e.target.value})} className="w-full px-4 py-3 bg-slate-50 border rounded-xl text-sm"/></div>
                         </div>
                         <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                             <h4 className="font-bold text-sm text-slate-700 mb-3">Perguntas</h4>
                             ...
                             <div className="space-y-2">{newSurvey.questions?.map((q, i) => <div key={i} className="p-3 bg-white border border-slate-200 rounded-lg text-sm font-medium flex justify-between items-center"><span className="flex-1">{i+1}. {q.text}</span><span className="text-xs text-slate-400 uppercase bg-slate-100 px-2 py-1 rounded">{q.type}</span></div>)}</div>
                         </div>
                     </div>
