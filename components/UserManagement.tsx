...
import { 
  ...
  Calendar, TrendingUp, TrendingDown, Clock, Download, FileDown, Heart, Mic, Send, ChevronLeft, ChevronRight, AlertCircle, ClipboardList
} from 'lucide-react';
...
const UserManagement: React.FC<UserManagementProps> = ({ systemInfo, templates, transactions, onUpdateTransactions }) => {
  ...
  const handleSaveSocialData = (socialData: SocialQuestionnaireData) => {
      if (editingUser) {
          setEditingUser({ ...editingUser, socialData });
          alert('Ficha Social salva temporariamente. Clique em "Salvar Alterações" no rodapé para persistir.');
      }
  };
    ...
    <div className="space-y-6">
      ...
      {editingUser && (
        ...
        <div className="bg-white w-full max-w-6xl h-[90vh] rounded-2xl shadow-2xl overflow-hidden flex flex-col animate-scale-in relative border border-slate-200">
          ...
          <div className="flex border-b border-slate-200 bg-white px-8 pt-2 overflow-x-auto">
              {[
                  { id: 'PERSONAL', icon: UserIcon, label: 'Informações Pessoais' }, 
                  { id: 'CONTACT', icon: Phone, label: 'Contato & Endereço' }, 
                  { id: 'FINANCE', icon: Wallet, label: 'Financeiro' }, 
                  { id: 'DOCS', icon: FileText, label: 'Documentos' }, 
                  { id: 'CARD', icon: CreditCard, label: 'Carteirinha' }
              ].map(tab => (<button key={tab.id} onClick={() => setActiveTab(tab.id as any)} className={`px-6 py-4 text-sm font-bold border-b-2 flex items-center gap-2 transition-all outline-none whitespace-nowrap ${activeTab === tab.id ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-800'}`}><tab.icon size={18}/>{tab.label}</button>))}
              {systemInfo.enableQuestionnaire && (
                  <button onClick={() => setActiveTab('SOCIAL')} className={`px-6 py-4 text-sm font-bold border-b-2 flex items-center gap-2 transition-all outline-none whitespace-nowrap ${activeTab === 'SOCIAL' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-800'}`}><ClipboardList size={18}/> Ficha Social</button>
              )}
          </div>
          <div className="flex-1 overflow-y-auto p-8 bg-slate-50/50 custom-scrollbar">
            {activeTab === 'SOCIAL' && systemInfo.enableQuestionnaire ? (
                <SocialQuestionnaire user={editingUser} onSave={handleSaveSocialData} onCancel={() => setActiveTab('PERSONAL')} />
            ) : (
                <>
                {activeTab === 'PERSONAL' && (
